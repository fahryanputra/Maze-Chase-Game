...
public class RIBSPlayerController : MonoBehaviour {
	...
	void OnTriggerEnter2D (Collider2D other) {
		if (other.tag == "Enemy") {
			...
			for (int i = 0; i < enemies.Length; i++) {
				...
				RIBSPathfinding resetEnemies = enemies [i].GetComponent<RIBSPathfinding> ();

				resetEnemies.currentNode = enemySpawn.GetComponent<Node> ();
				resetEnemies.isInEnemySpawn = true;
				resetEnemies.direction = Vector2.up;
				resetEnemies.targetNode = resetEnemies.currentNode.neighbours [0];
				resetEnemies.previousNode = resetEnemies.currentNode;
			}
