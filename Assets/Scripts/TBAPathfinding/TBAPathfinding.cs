...
public class TBAPathfinding : MonoBehaviour {
	...
	private float pathWeight = 0;

	public Text pathText;
	public Text nodeExpandedText;
	public Text pathWeightText;
	...
	void TBAFindPath () {
		...
		if (!solutionFound) {
			FindPath (GetNodeAtPosition (transform.localPosition).transform.position, GetTargetTile (), lookAhead);
			// Mathf.RoundToInt (GetDistance (transform.localPosition, player.transform.localPosition) / 30));
	...
	void FindPath (Vector2 startPos, Vector2 targetPos, int lookAhead) {
		...
		// int lookAhead = Mathf.RoundToInt (GetDistance (transform.localPosition, player.transform.localPosition) / 30); 
		...
		if (startNode != null && targetNode != null) {
			...
			while (openSet.Count > 0) {
				...
				// Debug.Log (openSet.Count + closeSet.Count);
				...
				*/
				...
				if (lookAheadCount == lookAhead) {
					...
					*/
					...
					bool nodeReset = GameObject.FindGameObjectWithTag ("Node").GetComponent<Node> ().nodeReset = true;
	...
	public void RetracePath (Node startNode, Node endNode) {
		...
		while (currentNode != startNode) {
			...
			pathWeight += currentNode.fCost;
			path.Add (currentNode);
			currentNode = currentNode.parent;
		}
		pathWeightText.text = "Path Weight\t: " + pathWeight.ToString ();
		path.Reverse ();
