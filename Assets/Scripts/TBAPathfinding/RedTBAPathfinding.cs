...
public class RedTBAPathfinding : MonoBehaviour {
	...
	void TBAFindPath () {
		...
		if (!solutionFound) {
			FindPath (GetNodeAtPosition (transform.localPosition).transform.position, GetTargetTile (), searchLimit);
			...
			while (openSet.Count > 0) {
				...
				*/
				...
				if (searchLimitCount == searchLimit) {
					...
					// Debug.Log ("Path found in: " + searchTime + " s with " + searchLimit + " lookahead");
					searchTime = 0;
					...
					// Debug.Log ("Player found in: " + searchTime + " s");
					searchTime = 0;
			...
			for (int i = 0; i < foundNodes.Length; i++) {
				if (foundNodesDirection [i] != Vector2.zero) {
					if (foundNodes [i] != previousNode) {
						...
						if (foundNodes [i].redFCost < leastredFCost || distance < leastDistance) {
