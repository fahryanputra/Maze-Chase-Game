...
public class BlueEnemyPathfinding : MonoBehaviour {
	...
	public EnemyType enemyType = EnemyType.Blue;
		...
		if (startNode != null && targetNode != null) {
			...
			while (openSet.Count > 0) {
				...
				for (int i = 1; i < openSet.Count; i++) {
					if (openSet [i].blueFCost < currentNode.blueFCost || openSet [i].blueFCost == currentNode.blueFCost && openSet [i].blueHCost < currentNode.blueHCost) {
				...
				for (int i = 0; i < currentNode.neighbours.Length; i++) {
					...
					if (newMovementCost < neighbour.blueGCost || !openSet.Contains (neighbour)) {
						neighbour.blueGCost = newMovementCost;
						neighbour.blueHCost = GetDistance (neighbour.transform.position, targetNode.transform.position);
			...
			for (int i = 0; i < foundNodes.Length; i++) {
				if (foundNodesDirection [i] != Vector2.zero) {
					if (foundNodes [i] != previousNode) {
						...
						if (foundNodes [i].blueFCost < leastblueFCost || distance < leastDistance) {
							leastblueFCost = foundNodes [i].blueFCost;
