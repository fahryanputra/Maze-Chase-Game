...
public class OrangeEnemyPathfinding : MonoBehaviour {
	...
	public EnemyType enemyType = EnemyType.Orange;
		...
		if (startNode != null && targetNode != null) {
			...
			while (openSet.Count > 0) {
				...
				for (int i = 1; i < openSet.Count; i++) {
					if (openSet [i].orangeFCost < currentNode.orangeFCost || openSet [i].orangeFCost == currentNode.orangeFCost && openSet [i].orangeHCost < currentNode.orangeHCost) {
				...
				for (int i = 0; i < currentNode.neighbours.Length; i++) {
					...
					if (newMovementCost < neighbour.orangeGCost || !openSet.Contains (neighbour)) {
						neighbour.orangeGCost = newMovementCost;
						neighbour.orangeHCost = GetDistance (neighbour.transform.position, targetNode.transform.position);
			...
			for (int i = 0; i < foundNodes.Length; i++) {
				if (foundNodesDirection [i] != Vector2.zero) {
					if (foundNodes [i] != previousNode) {
						...
						if (foundNodes [i].orangeFCost < leastorangeFCost || distance < leastDistance) {
							leastorangeFCost = foundNodes [i].orangeFCost;
